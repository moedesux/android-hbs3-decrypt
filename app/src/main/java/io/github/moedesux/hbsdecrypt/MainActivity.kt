...
class MainActivity : ComponentActivity() {
    ...
    private fun start() { val s = source ?: return; val d = destination ?: return; cancelRequested = false; state = state.copy(running = true, outcome = null, bytesRead = 0, counts = RecoveryCounts()); window.addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON); RecoveryRunner(contentResolver, Executors.newSingleThreadExecutor()).runTree(s, d, password.toCharArray(), policy = state.collisionPolicy, cancellation = DecryptionCancellation { cancelRequested }, onProgress = { runOnUiThread { state = state.copy(bytesRead = it) } }, onResult = { result -> runOnUiThread { password = ""; state = state.copy(running = false, passwordPresent = false, outcome = result); window.clearFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON) } }, onComplete = { counts -> runOnUiThread { state = state.copy(counts = counts) } }) }
