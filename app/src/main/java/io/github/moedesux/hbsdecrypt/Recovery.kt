...
enum class SourceKind { FILE, FOLDER }
...
data class RecoveryState(val source: Uri? = null, val destination: Uri? = null, val passwordPresent: Boolean = false, val running: Boolean = false, val bytesRead: Long = 0, val outcome: String? = null, val collisionPolicy: CollisionPolicy = CollisionPolicy.SKIP, val counts: RecoveryCounts = RecoveryCounts(), val sourceKind: SourceKind = SourceKind.FILE, val sourceName: String? = null, val destinationName: String? = null) { val canStart get() = source != null && destination != null && passwordPresent && !running }

class RecoveryRunner(private val resolver: ContentResolver, private val executor: Executor) {
    fun run(source: Uri, tree: Uri, password: CharArray, policy: CollisionPolicy = CollisionPolicy.SKIP, cancellation: DecryptionCancellation = DecryptionCancellation { false }, onProgress: (Long) -> Unit, onResult: (String) -> Unit, onComplete: (RecoveryCounts) -> Unit = {}) = executor.execute {
        runFiles(listOf(FileEntry(source, listOf(displayName(source)))), tree, password, policy, cancellation, onProgress, onResult, onComplete, legacyResult = true)
    }
    ...
    private fun entries(tree: Uri): List<FileEntry> { val out = mutableListOf<FileEntry>(); fun walk(parent: Uri, parts: List<String>) { val children = DocumentsContract.buildChildDocumentsUriUsingTree(tree, documentId(parent)); resolver.query(children, arrayOf(DocumentsContract.Document.COLUMN_DOCUMENT_ID, DocumentsContract.Document.COLUMN_DISPLAY_NAME, DocumentsContract.Document.COLUMN_MIME_TYPE), null, null, null)?.use { c -> while (c.moveToNext()) { val cid = c.getString(0); val name = c.getString(1); val uri = DocumentsContract.buildDocumentUriUsingTree(tree, cid); if (c.getString(2) == DocumentsContract.Document.MIME_TYPE_DIR) walk(uri, parts + name) else out += FileEntry(uri, parts + name) } } }; walk(documentUri(tree), emptyList()); return out }
    private fun child(parent: Uri, name: String): Uri? { val u = DocumentsContract.buildChildDocumentsUriUsingTree(parent, documentId(parent)); return resolver.query(u, arrayOf(DocumentsContract.Document.COLUMN_DOCUMENT_ID, DocumentsContract.Document.COLUMN_DISPLAY_NAME), null, null, null)?.use { c -> while (c.moveToNext()) if (c.getString(1) == name) return@use DocumentsContract.buildDocumentUriUsingTree(parent, c.getString(0)); null } }
    private fun ensureDirectory(root: Uri, parts: List<String>): Uri { var current = documentUri(root); parts.forEach { name -> current = child(current, name) ?: DocumentsContract.createDocument(resolver, current, DocumentsContract.Document.MIME_TYPE_DIR, name) ?: error("Could not create directory $name") }; return current }
    private fun documentId(uri: Uri): String = if (uri.pathSegments.contains("document")) DocumentsContract.getDocumentId(uri) else DocumentsContract.getTreeDocumentId(uri)
    private fun documentUri(uri: Uri): Uri = if (uri.pathSegments.contains("document")) uri else DocumentsContract.buildDocumentUriUsingTree(uri, DocumentsContract.getTreeDocumentId(uri))
}
